 ...
 */
SIZE_TYPE(size_align)(SIZE_TYPE(n))
{
	return (n + sizeof(PTR_TYPE()) - 1) & ~(sizeof(PTR_TYPE()) - 1);
}

BOOL_TYPE(array_init)(ARRAY_TYPE(*self), size_t size)
{
...
ARRAY_TYPE(array_create)(SIZE_TYPE(elt_size), SIZE_TYPE(n), void (*free)(void *))
{
...
ARRAY_TYPE(array_seize_buffer)(PTR_TYPE(*buffer), SIZE_TYPE(bufsize), SIZE_TYPE(elt_size), SIZE_TYPE(n),
 void (*_free)(void *))
{
	HR_COMPLAIN_IF(elt_size == 0);
	HR_COMPLAIN_IF(SIZE_T_SAFE_TO_MUL(elt_size, n) == false);
...
(RDONLY_ARRAY_TYPE(src), SSIZE_TYPE(start), SSIZE_TYPE(end))
{
	...
	SIZE_TYPE(n_elems) = labs((long)(start - end));
